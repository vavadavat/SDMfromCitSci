...
#DATA
load(paste0(in.dir,"full.dat.RData"))
...
##bestmodels
tm.dat <- dat[which(dat$group %in% "tidalMarsh"),]
...
####Klingbeil
############
f.dat <- dat[which(dat$group %in% "forest"),]
...
s.dat <- dat[which(dat$group %in% "shrub"),]
...
#####BBS
############
w.dat <- dat[which(dat$group %in% "woodland"),]
