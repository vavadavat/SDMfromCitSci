...
run.models <- function(cov.names,bird.names,training.list,test.dat,covariates,save.name){
  ...
  eval.testdata.list <- list()
  
  for(j in 1:length(training.list)){
    ...
    dat.all <-merge(training.list[[j]],covariates,by=c("LATITUDE","LONGITUDE"),all.x=F,all.y=F)
    n.obs <- nrow(dat.all)
    #reformat bird names to be consistent
    names <- colnames(dat.all)[5:480]
    names <- gsub(pattern=" ",replacement=".",x=names)
    names <- gsub(pattern="-",replacement=".",x=names)
    names <- gsub(pattern="'",replacement=".",x=names)
    colnames(dat.all)[5:480] <- names
    ...
    eval.testdata.list[[j]] <- eval.testdata.dat
  }
  
  names(eval.testdata.list) <- training.list
  return(eval.testdata.list)
}
